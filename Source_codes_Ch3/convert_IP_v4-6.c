...
void convert_ipv6(const char *ip_str) {
    ...
    if (inet_pton(AF_INET6, ip_str, &ipv6_addr) == 1) {
        printf("inet_pton (IPv6): Successfully converted IP address:\n");
        // In dạng nhị phân (128 bit = 16 byte)
    printf("IPv6 binary form:\n");
    for (int i = 0; i < 16; i++) {
        for (int b = 7; b >= 0; b--) {
            printf("%d", (ipv6_addr.s6_addr[i] >> b) & 1);
        }
        printf(" "); // cách mỗi byte 1 khoảng trắng
    }
    printf("\n");
    } else {
